...
LEFT JOIN music_db.songs ON artist.Artist_ID = songs.Artist_ID
...
//  Table header. 
 ...
 ';
  ...
  while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	  echo '<tr><td>' . $row['Song_Name'] . '</td><td>' . $row['First_name'] .
	  '</td><td>' . $row['Release_date'] . '</td><td>' . $row['Genre'] . 
	  '</td><td>' . $row['Price'] . '</td><td>' . $row['Nationality'] . 
